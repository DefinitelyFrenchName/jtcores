...
// ---------------------------------------------------------------------------
// CPS-2 WIDE (Vampire Saved) OVERRIDE of cores/cps2/hdl/jtcps2_main.v.
// SLICE D4 — THE 6 MB PROGRAM WINDOW. Copied at jtcores v1.7.3; the file is
// SHARED with cores/cps2, so editing it in place would change the reference
// core. The diff against the original IS the trust surface (CLAUDE.md rule 1
// v2) and is frozen line by line by tests/test_mister_wide_gate.sh.
//
// WHAT CHANGES. Stock CPS-2 decodes a FLAT 4 MB of program ROM,
// `rom_cs <= A[23:22] == 2'b00`, and CPS-2 WIDE v1 declares 6 MB
// (docs/project/mister_map.md section 8). Three lines:
//   * `rom_cs` gains `$400000-$5FFFFF`, READS ONLY;
//   * `rom_addr` widens from A[21:1] to A[22:1];
//   * `one_wait`'s boundary moves from 4'h5 to 4'h6.
// All three are ANDed with / selected by `wide_en`, so with the profile clear
// the expressions are the reference core's character for character.
//
// WHY READ-ONLY, AND WHY THAT IS ENOUGH. `objcfg_cs` at :190 decodes a WHOLE
// megabyte at `$400000-$4FFFFF` — the RTL is looser than the hardware — but it
// is qualified `&& !RnW`. So a READ anywhere in that window asserts nothing
// today and asserts only `rom_cs` after this change: there is no read
// collision, and a WRITE still reaches only `objcfg_cs`. `cps2_wide.md`
// reserves `$400000-$40000F` and forbids allocating there, which is what makes
// that safe; the build's allocator honours it (`wide_ext` starts at
// `0x400010`).
//
// THE WAIT STATE IS NOT COSMETIC AND IT SHIPS HERE ON PURPOSE. `:167` gives
// `A[23:20] < 4'h5` one wait state, so the first megabyte of the extension
// would be one-wait like every other byte of program ROM while the SECOND
// (`$500000-$5FFFFF`) would be ZERO-wait — a different bus timing for part of
// the same region. Today the profile puts only the 30-byte facing-alias thunk
// at `$5FFF00` there, so nothing would be seen, which is exactly how this kind
...
// CPS-2 WIDE: below 5 MB and above 8 MB on stock; below SIX on the profile,
// so the second megabyte of the program extension runs at the same bus timing
// as every other byte of program ROM. See the header.
always @(*) begin
    one_wait = !ASn && BGACKn &&
        (A[23:20] < (wide_en ? 4'h6 : 4'h5) || A[23:20]>=4'h8);
end
...
always @(posedge clk, posedge rst) begin
    if( rst ) begin
        ...
        if( !ASn && BGACKn ) begin // PAL PRG1 12H
            rom_addr    <= A[22:1];
            // CPS-2 WIDE: the stock flat 4 MB, plus $400000-$5FFFFF on READS
            // only. The objcfg port above overlaps the first megabyte but is
            // qualified !RnW, so there is no collision. See the header.
